...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Email`: `character varying`, `PWD`: `character varying`, `TokenExpiresIn`: `bigint`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
